...
{      
    ...
    {      
        ...
        ROS_INFO_STREAM("Cup diameter is:" << cup_image_diameter << "   actual diameter is : " << image_diameter*2);
        ROS_INFO_STREAM("cup_measure_diameter is:" << cup_measure_diameter << "   actual cup_diameter is : " << cup_diameter);
        ROS_DEBUG_STREAM("输出三维坐标" << d); 

        // 满足要求
        if(((cup_image_diameter - image_diameter) < diameter_max_error_ && cup_image_diameter > image_diameter) || ((image_diameter - cup_image_diameter) < diameter_max_error_ && cup_image_diameter <= image_diameter))
        {
